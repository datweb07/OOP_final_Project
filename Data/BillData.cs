...
namespace OOP_finalProject
{
    ...
    {
        private string pathJson = Path.Combine(GetPath.path, nameof(Bill) + ".json");
        ...
        public List<Bill> GetData()
        {
            if (File.Exists(pathJson))
            {
                try
                {
                    ...
                    List<Bill> bills = JsonSerializer.Deserialize<List<Bill>>(jsonString, options);
                    return bills;
                }
                catch (Exception ex)
                {
        ...
        public void SaveData(List<Bill> bills)
        {
            try
            {
                ...
                string jsonString = JsonSerializer.Serialize(bills, options);
